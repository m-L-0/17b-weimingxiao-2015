...
# print(X_train.shape)
# print(y_train.shape)
[X_val, y_val] = read.read_and_decode('/home/r/mycar/test.tfrecords',4314)
...
with tf.Session() as sess:
    ...
    if ckpt:
        ...
        # check_point_path = '/home/r/mycar/cnn1' # 保存好模型的文件路径
...
for i in range(4314):
    if Y[i]<26:
        if np.argmax(y_batch[i])<26:
            fp.write('###### '+'预测为'+str(chr(ord('A') + int(Y[i])))+'实际为'+str(chr(ord('A') + int(np.argmax(y_batch[i]))))+"\n")
        else:
            fp.write('###### '+'预测为'+str(chr(ord('A') + int(Y[i])))+'实际为'+str(chr(ord('0') + int(np.argmax(y_batch[i]))-26))+"\n")
    if Y[i]>25:
        if np.argmax(y_batch[i])<26:
            fp.write('###### '+'预测为'+str(chr(ord('0') + int(Y[i]) - 26))+'实际为'+str(chr(ord('A') + int(np.argmax(y_batch[i]))))+"\n")
        else:
            fp.write('###### '+'预测为'+str(chr(ord('0') + int(Y[i]) - 26))+'实际为'+str(chr(ord('0') + int(np.argmax(y_batch[i])) - 26))+"\n")
fp.close()
# fp = open("train.txt", "w+")
# for j in range(3000):
#     fp.write(str(np.argmax(y_batch[j]))+'\n')
...
# 全部训练完了再做测试，batch_size=100
    # for i in range(100): 
    #     X_batch,y_batch = chooseone(X_val, y_val,200)
