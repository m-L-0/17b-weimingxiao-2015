...
def cnn(a):
    def image(a):
        ...
        w=np.array(im)
        w=w/255
        images = np.empty((1, x_s*y_s))
        for q in range(1):
            images[q] = w.reshape(x_s*y_s)
        return images
    ...
    # 全部训练完了再做测试，batch_size=100
        for i in range(1): 
            ...
            Ytemp = max_idx_p.eval(feed_dict={X_: X_batch, keep_prob: 1.0})
            # if (i+1) % 100 == 0:
            s=list()
            for i in range(4):
                if Ytemp[0][i]!=10:
                    s.append(str(Ytemp[0][i]))
            # print(s)
...
# cnn('/home/r/captcha/data/captcha/images/8.jpg')